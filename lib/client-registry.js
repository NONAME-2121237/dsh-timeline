		...
		function TimelineOverlay(props) {
			...
			*  到所点轮次再弹回，页面滚动时时间线反而不像在连续滚动。 */
			...
			const zoneOf = (e) => {
				...
				if (e.clientX >= rect.left - HOVER_KEEP_RANGE && e.clientX <= rect.right + HOVER_KEEP_RANGE && e.clientY >= rect.top - HOVER_KEEP_RANGE && e.clientY <= rect.bottom + HOVER_KEEP_RANGE) return "near";
				return "out";
			};
			(0, react.useEffect)(() => {
				const onMove = (e) => {
