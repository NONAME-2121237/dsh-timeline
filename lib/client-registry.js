		...
		function TimelineOverlay(props) {
			...
			*  到所点轮次再弹回，页面滚动时时间线反而不像在连续滚动。 */
			...
			const zoneOf = (e) => {
				...
				const extLeftX = pos === null ? rect.left : window.innerWidth - pos.right - LINE_W / 2 - HEXT_RADIUS;
			...
			const hoveredIndex = (() => {
				...
				const near = clamp(Math.round((cursorYRef.current - 6.5) / LINE_STEP), 0, Math.max(0, count - 1));
				return Math.abs(cursorYRef.current - (near * LINE_STEP + 6.5)) <= HOVER_NEAR ? near : null;
			})();
			...
			let tipNode = null;
			if (tipIndex !== null && turns[tipIndex] !== void 0) {
